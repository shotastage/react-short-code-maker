...
export class ContextProcessor extends React.Component {
  ...
  youtube(code) {
    ...
    try {
      let playbackURL = "https://www.youtube.com/embed/" + code.match(/id\=\".*?(?: .+?)?\"/g)[0].replace( /id\=\"/g , "").replace("\"", "")
  ...
  apple(code) {
    try {
      let playbackURL = "https://embed.music.apple.com/jp/album/" + code.match(/url\=\".*?(?: .+?)?\"/g)[0].replace( /url\=\"/g , "").replace("\"", "").split("album/")[1] + "&at=1001lqeA"
